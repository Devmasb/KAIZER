...
def actualizar_nivel_y_monto(trade_data, user_input, step_amount):
    global last_win,trade_direction
    resultado = trade_data['result']
    step = trade_data['step']
    monto_actual = step_amount[step]
    
    if resultado == 'loss':
        # Incrementar nivel tras una pérdida
        step += 1
        nuevo_monto = step_amount[step] if step < len(step_amount) else step_amount[-1]  # Asegurar que no supere los niveles definidos
        last_win = False  # Reiniciar el seguimiento de ganancias consecutivas
    else:
        # Verificar si es la primera o segunda ganancia consecutiva
        if last_win == True:  # Si ya hubo una ganancia previa
            # Reiniciar tras dos ganancias consecutivas
            step = 0
            nuevo_monto = step_amount[step]
            last_win = False
        else:
            # Compuesto: Ganancia * 1.90
            nuevo_monto = monto_actual * 1.90
            last_win = True  # Marcar que es la primera ganancia consecutiva


    trade_data['step'] = step
    return nuevo_monto
...
async def strategies(user_input={}, instruments_list={}, trade_data={}, order_type='', candle_data=[]):
    ...
    if order_type == "closed_order":
        ...
        nuevo_monto = actualizar_nivel_y_monto(trade_data, user_input, user_input['bet_amounts'])
