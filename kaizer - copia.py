...
# 🔄 Ciclo principal del bot con estrategia Labouchere Clásico
# 🔄 Ciclo principal del bot con Labouchere clásico aplicado a sesiones
async def trade_loop():
    ESCALADO_FACTOR = ESCALADO_FACTOR_GLOBAL
    SECUENCIA_SESIONES = [1, 2, 3]
    MONTO_MAXIMO_OPERACION = 15.0
    resultados = []
    ...
    print(f"📊 Estrategia: LABOUCHERE POR SESIÓN | Secuencia inicial: {SECUENCIA_SESIONES}\n")
    ...
    while len(SECUENCIA_SESIONES) > 0:
        ...
        monto_operacion = int(take_profit_sesion /2.5)
        ...
        while True:
            print(f"\n📌 Sesión {sesion_actual} | TP: {take_profit_sesion:.2f} | SL: {stop_loss_sesion:.2f} | Monto fijo: {monto_operacion:.2f}")
            print(f"\n📌 PATRÓN RESULTADOS: {resultados}")
            # Buscar activo y ejecutar operación válida
            while True:
                ...
                if result in ["Doji", "Failed"]:
                    print(f"⚠️ Operación inválida ({result}). Reintentando con nuevo activo y mismo monto...")
            ...
            if saldo_sesion <= stop_loss_sesion:
                ...
                #SECUENCIA_SESIONES.append(take_profit_sesion)
